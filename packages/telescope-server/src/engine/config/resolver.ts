 ...
 * @example
 * ```typescript
 * import { resolveConfig, materializeRules, materializeExtensions } from "telescope-server";
 *
 * // Load configuration
		...
		if (
			...
			return {
				...
				error: {
					...
					message: `Rule in ${resolvedPath} has invalid ruleType "${candidateRule.meta.ruleType ?? "undefined"}". Expected "openapi". Rules should be created using defineRule() from the telescope-server package.`,
					path,
					resolvedPath,
				},
			};
