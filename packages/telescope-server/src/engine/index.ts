...
import type { LintingContext } from "./context/context-resolver.js";
...
export { DocumentTypeCache } from "./context/document-cache.js";
export {
	findSharedSchemas,
	type MultiRootContext,
	resolveMultipleRoots,
} from "./context/multi-root-handler.js";
export { ProjectContextCache } from "./context/project-cache.js";
export {
	discoverWorkspaceRoots,
	findRootDocumentsForPartial,
} from "./context/root-discovery.js";
export { loadDocument } from "./load-document.js";
export { matchesPattern } from "./pattern-matcher.js";
export {
	loadGenericRule,
 ...
 * Accepts either a callback function that receives the `z` builder,
 * or a schema directly. Using the callback pattern is recommended as
 * it eliminates the need for users to import zod as a separate dependency.
 *
 * @param schemaOrCallback - A Zod schema or a callback that receives `z` and returns a schema
 * @returns The Zod schema
 *
 * @example
 * import { defineSchema } from "telescope-server";
 *
 * // Recommended: Use callback pattern (no need to import zod)
 ...
 *
 * @example
 * import { defineSchema, z } from "telescope-server";
 *
 * // Alternative: Use z directly from telescope-server
...
export function defineSchema<T extends z.ZodType>(
	schemaOrCallback: T | ((zod: typeof z) => T),
): T {
	if (typeof schemaOrCallback === "function") {
		return (schemaOrCallback as (zod: typeof z) => T)(z);
	}
