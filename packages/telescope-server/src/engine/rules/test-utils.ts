...
import type { Diagnostic, ProjectContext } from "../index.js";
import {
	buildIndex,
	buildRefGraph,
	getValueAtPointer,
...
// Get test-files directory path - from packages/telescope-server/src/engine/rules to packages/test-files/openapi
...
/**
 * Helper to find diagnostics by rule code
 */
export function findDiagnostics(
	...
	return diagnostics.filter(
		(d) => d.code === ruleCode || d.code.includes(ruleCode),
	);
}
...
export function hasDiagnostic(
	...
	return diagnostics.some(
		(d) =>
			(d.code === ruleCode || d.code.includes(ruleCode)) &&
