...
export class Session implements vscode.Disposable {
	...
	private async startClient(): Promise<void> {
		...
		const clientOptions: LanguageClientOptions = {
			...
			documentSelector: [
				{ language: "yaml", pattern: `${this.workspaceFolder.uri.fsPath}/**/*` },
				{ language: "json", pattern: `${this.workspaceFolder.uri.fsPath}/**/*` },
				{ language: "openapi-json", pattern: `${this.workspaceFolder.uri.fsPath}/**/*` },
				{ language: "openapi-yaml", pattern: `${this.workspaceFolder.uri.fsPath}/**/*` },
			],
		...
		const clientId = `aperture-${this.workspaceFolder.name}`;
		const clientName = `Aperture (${this.workspaceFolder.name})`;
	...
	private setupFileWatchers(): void {
		...
		const configWatcher = vscode.workspace.createFileSystemWatcher(
			new vscode.RelativePattern(this.workspaceFolder, ".telescope/config.yaml"),
		);
	...
	private async runScan(): Promise<void> {
		...
		try {
			...
			this.log(`Scan complete: ${openAPIFiles?.length || 0} OpenAPI files found`);
	...
	private async sendOpenAPIFilesToServer(files: string[]): Promise<void> {
		...
		try {
			const result = await this.client.sendRequest("aperture/setOpenAPIFiles", {
				files,
			});
			this.log(`Sent ${files.length} files to server: ${JSON.stringify(result)}`);
		} catch (error) {
	...
	private notifyServerFileChange(
		...
		try {
			this.client.sendNotification("aperture/notifyFileChange", { uri, type });
		} catch (error) {
