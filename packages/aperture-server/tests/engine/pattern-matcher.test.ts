...
describe("Pattern Matching", () => {
	...
	describe("Exact file path matches", () => {
		it("should match exact file path", () => {
			...
			expect(matchesPattern(uri, ["test.yaml"], workspaceRoots)).toBe(true);
		});

		it("should not match different file path", () => {
			const uri = URI.file(resolve(workspaceRoot, "other.yaml")).toString();
			expect(matchesPattern(uri, ["test.yaml"], workspaceRoots)).toBe(false);
		});
		...
		it("should match file path with subdirectory", () => {
			...
			expect(matchesPattern(uri, ["schemas/test.yaml"], workspaceRoots)).toBe(
				true,
			);
		});
	});
	...
	describe("Glob patterns", () => {
		it("should match **/*.yaml pattern", () => {
			const uri = URI.file(resolve(workspaceRoot, "test.yaml")).toString();
			expect(matchesPattern(uri, ["**/*.yaml"], workspaceRoots)).toBe(true);
		});
		...
		it("should match *.json pattern", () => {
			...
			expect(matchesPattern(uri, ["*.json"], workspaceRoots)).toBe(true);
		});
		...
		it("should match **/schemas/** pattern", () => {
			...
			expect(matchesPattern(uri, ["**/schemas/**"], workspaceRoots)).toBe(true);
		});
		...
		it("should match pattern with character class", () => {
			...
			expect(matchesPattern(uri, ["test[0-9].yaml"], workspaceRoots)).toBe(
				true,
			);
		});
		...
		it("should match pattern with braces", () => {
			const uri = URI.file(resolve(workspaceRoot, "test.yaml")).toString();
			expect(matchesPattern(uri, ["test.{yaml,yml}"], workspaceRoots)).toBe(
				true,
			);
		});
	});
	...
	describe("Exclude patterns", () => {
		...
		it("should exclude files matching exclude pattern with ! prefix", () => {
			...
			// Wait, the new logic uses ! prefix for exclusion in the single patterns array.
			expect(
				matchesPattern(
					uri,
					["**/*.yaml", "!**/node_modules/**"],
	...
	describe("Config file exclusion", () => {
		it("should never match .telescope/config.yaml", () => {
			...
			expect(matchesPattern(uri, ["**/*.yaml"], workspaceRoots)).toBe(false);
		});
		...
		it("should not match config file even in nested workspace", () => {
			...
			expect(matchesPattern(uri, ["**/*.yaml"], [nestedRoot])).toBe(false);
		});
	});
	...
	describe("Workspace root resolution", () => {
		it("should resolve relative paths from workspace root", () => {
			...
			expect(matchesPattern(uri, ["subdir/test.yaml"], workspaceRoots)).toBe(
				true,
			);
		});
		...
		it("should handle multiple workspace roots", () => {
			...
			expect(matchesPattern(uri, ["test.yaml"], [root1, root2])).toBe(true);
		});

		it("should handle workspace root with trailing slash", () => {
			const uri = URI.file(resolve(workspaceRoot, "test.yaml")).toString();
			expect(matchesPattern(uri, ["test.yaml"], [`${workspaceRoot}/`])).toBe(
				true,
			);
		});
	});
	...
	describe("Edge cases", () => {
		...
		it("should handle empty include patterns (defaults to YAML/JSON)", () => {
			const uri = URI.file(resolve(workspaceRoot, "test.yaml")).toString();
			expect(matchesPattern(uri, undefined as any, workspaceRoots)).toBe(true);
		});
		...
		it("should handle empty include patterns for non-YAML/JSON files", () => {
			...
			expect(matchesPattern(uri, undefined as any, workspaceRoots)).toBe(false);
		});
		...
		it("should handle pattern with escaped characters", () => {
			...
			expect(typeof matchesPattern(uri, ["test[*].yaml"], workspaceRoots)).toBe(
				"boolean",
			);
		});
	});
	...
	describe("Pattern inheritance in additional validation groups", () => {
		it("should match file when group pattern matches", () => {
			...
			expect(matchesPattern(uri, groupPatterns, workspaceRoots)).toBe(true);
		});
		...
		it("should not match file when group pattern does not match", () => {
			...
			const groupPatterns = ["**/schemas/**"];
			expect(matchesPattern(uri, groupPatterns, workspaceRoots)).toBe(false);
		});
		...
		it("should respect schema-specific pattern override", () => {
			...
			expect(matchesPattern(uri, [schemaPattern], workspaceRoots)).toBe(true);
		});
		...
		it("should respect rule-specific pattern override", () => {
			...
			expect(matchesPattern(uri, [rulePattern], workspaceRoots)).toBe(true);
		});
	});
});
