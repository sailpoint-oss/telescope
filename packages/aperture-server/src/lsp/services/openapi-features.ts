...
function findAllRefNodes(node: IRNode): Array<{ node: IRNode; ref: string }> {
	...
	if (node.kind === "string" && node.key === "$ref" && typeof node.value === "string") {
...
function generateOperationId(vc: OpenAPIVirtualCode, range: Range): string {
	...
	for (let i = range.start.line; i >= 0 && i > range.start.line - 20; i--) {
		...
		const methodMatch = line.match(/^\s*(get|post|put|delete|patch|head|options|trace):/i);
		if (methodMatch?.[1]) {
 ...
 */
function getTagCompletions(virtualCode: OpenAPIVirtualCode, uri: string): CompletionItem[] {
	const items: CompletionItem[] = [];
	const parsedObject = virtualCode.parsedObject as Record<string, unknown>;
	const tags = parsedObject.tags as Array<{ name: string; description?: string }> | undefined;
...
function getResponseCodeCompletions(): CompletionItem[] {
	return [
		...
		{ label: "422", kind: 12 as CompletionItemKind, detail: "Unprocessable Entity" },
		{ label: "500", kind: 12 as CompletionItemKind, detail: "Internal Server Error" },
		{ label: "default", kind: 12 as CompletionItemKind, detail: "Default response" },
	];
}
...
function getMediaTypeCompletions(): CompletionItem[] {
	return [
		{ label: "application/json", kind: 12 as CompletionItemKind, detail: "JSON" },
		{ label: "application/xml", kind: 12 as CompletionItemKind, detail: "XML" },
		{ label: "application/x-www-form-urlencoded", kind: 12 as CompletionItemKind, detail: "Form data" },
		{ label: "multipart/form-data", kind: 12 as CompletionItemKind, detail: "Multipart form" },
		{ label: "text/plain", kind: 12 as CompletionItemKind, detail: "Plain text" },
		{ label: "text/html", kind: 12 as CompletionItemKind, detail: "HTML" },
		{ label: "application/octet-stream", kind: 12 as CompletionItemKind, detail: "Binary" },
		...
		{ label: "image/jpeg", kind: 12 as CompletionItemKind, detail: "JPEG image" },
	];
}
...
function addRequiredHints(
	...
	if (node.kind === "object" && node.children) {
		...
		if (requiredNode?.kind === "array" && requiredNode.children && propertiesNode?.children) {
			const requiredNames = new Set(
				requiredNode.children.filter((c) => typeof c.value === "string").map((c) => c.value as string),
			);
			...
			for (const propNode of propertiesNode.children) {
				if (propNode.key && requiredNames.has(propNode.key) && propNode.loc) {
					...
					if (propRange && propRange.start.line >= range.start.line && propRange.start.line <= range.end.line) {
						hints.push({
...
function getTokenInfo(node: IRNode): TokenInfo | null {
	...
	if (["get", "post", "put", "delete", "patch", "head", "options", "trace"].includes(node.key ?? "")) {
	...
	if (node.key === "type" && node.kind === "string" && typeof node.value === "string") {
	...
	if (node.ptr?.startsWith("#/components/schemas/") && node.key && !node.ptr.includes("/properties/")) {
		// Only match direct schema children, not nested properties
		const parts = node.ptr.split("/");
		if (parts.length === 4) { // #/components/schemas/SchemaName
...
function collectSemanticTokens(
	...
	if (tokenInfo) {
		...
		const position = tokenInfo.target === "key"
