...
import {
	buildLineOffsets,
	decodedPositionToSource,
	decodedRangeToSourceRange,
	type JsonStringMapping,
	jsonStringEncode,
	MarkdownVirtualCode,
	sourcePositionToDecoded,
} from "../languages/virtualCodes";
import type { ApertureVolarContext } from "../workspace/context";
import { matchDocument } from "./shared/virtual-code-utils.js";

/**
 * Context for JSON position translation.
 * Contains all the information needed to convert between source and decoded positions.
 */
interface JsonMappingContext {
	/** The JSON string mapping for position translation */
	mapping: JsonStringMapping;
	/** Line offsets in the source document */
	sourceLineOffsets: number[];
	/** Line offsets in the decoded content */
	decodedLineOffsets: number[];
	/** The segment index in the MarkdownVirtualCode */
	segmentIndex: number;
	/** The MarkdownVirtualCode instance */
	virtualCode: MarkdownVirtualCode;
}
...
export function create({
	...
	const logger = shared.getLogger("Markdown Service");
	logger.log("Creating markdown service plugin");
	return {
		name: "telescope-markdown-service",
		capabilities: {
			codeActionProvider: {
		...
		create(context): LanguageServicePluginInstance<Provide> {
			...
			/**
			 * Get the MarkdownVirtualCode for a document, if it exists and has JSON mappings.
			 */
			function getMarkdownVirtualCode(
				document: TextDocument,
			): MarkdownVirtualCode | undefined {
				...
				if (!sourceScript?.generated) return undefined;

				const virtualCode =
					sourceScript.generated.embeddedCodes.get(embeddedCodeId);
				if (virtualCode instanceof MarkdownVirtualCode) {
					return virtualCode;
				}
				return undefined;
			}
			...
			/**
			 * Get the JSON mapping context for a document if it's embedded in a JSON source.
			 */
			function getJsonMappingContext(
				document: TextDocument,
			): JsonMappingContext | undefined {
				const virtualCode = getMarkdownVirtualCode(document);
				if (!virtualCode || !virtualCode.hasJsonMappings()) {
					return undefined;
				}

				// Get the first JSON mapping (typically there's one segment per description)
				const firstMapping = virtualCode.jsonMappings.get(0);
				if (!firstMapping) return undefined;

				// Build line offsets for position conversion
				const sourceText = virtualCode.segments[0]?.content ?? "";
				const decodedLineOffsets = buildLineOffsets(
					firstMapping.decodedContent,
				);

				// For source line offsets, we need to get the raw source text
				// Since we're working within the virtual code, we use the decoded content's structure
				// The actual source offsets are absolute in the parent document
				const sourceLineOffsets = buildLineOffsetsFromMapping(firstMapping);
			...
			/**
			 * Build source line offsets from a JSON mapping by walking through escape sequences.
			 */
			function buildLineOffsetsFromMapping(
				mapping: JsonStringMapping,
			): number[] {
				// For JSON strings, source doesn't have actual newlines
				// Lines in decoded content correspond to \n sequences in source
				// We return [0] as there's conceptually one "line" in source
				return [0];
			}

			/**
			 * Convert a decoded position to source position using JSON mapping.
			 */
			function convertDecodedToSourcePosition(
				mappingCtx: JsonMappingContext,
				decodedPos: Position,
			): Position {
				return decodedPositionToSource(
					mappingCtx.mapping,
					decodedPos,
					mappingCtx.decodedLineOffsets,
					mappingCtx.sourceLineOffsets,
				);
			}

			/**
			 * Convert a source position to decoded position using JSON mapping.
			 */
			function convertSourceToDecodedPosition(
				mappingCtx: JsonMappingContext,
				sourcePos: Position,
			): Position {
				return sourcePositionToDecoded(
					mappingCtx.mapping,
					sourcePos,
					mappingCtx.sourceLineOffsets,
					mappingCtx.decodedLineOffsets,
			...
			/**
			 * Convert a decoded range to source range using JSON mapping.
			 */
			function convertDecodedToSourceRange(
				mappingCtx: JsonMappingContext,
				decodedRange: Range,
			): Range {
				return decodedRangeToSourceRange(
					mappingCtx.mapping,
					decodedRange,
					mappingCtx.decodedLineOffsets,
					mappingCtx.sourceLineOffsets,
			...
			return {
				...
				async resolveCodeAction(codeAction, token) {
					if (codeAction.kind === organizeLinkDefKind) {
						...
						const edits = await mdLs.organizeLinkDefinitions(
							document,
							{ removeUnused: true },
							token,
						);
				...
				async provideCompletionItems(document, position, _context, token) {
					if (!prepare(document)) return;

					// Check if this is embedded in a JSON document
					const mappingCtx = getJsonMappingContext(document);

					let effectivePosition = position;
					if (mappingCtx) {
						// For JSON: convert source position to decoded position
						effectivePosition = convertSourceToDecodedPosition(
							mappingCtx,
							position,
						);
					}
					...
					// Convert completion item ranges if needed
					if (mappingCtx && items) {
						for (const item of items) {
							if (item.textEdit && "range" in item.textEdit) {
								item.textEdit.range = convertDecodedToSourceRange(
									mappingCtx,
									item.textEdit.range,
								);
							}
				...
				async provideDiagnostics(document, token) {
					...
					const diags = await mdLs.computeDiagnostics(
						document,
						configuration,
						token,
					);
					...
					if (mappingCtx && diags.length > 0) {
						// Convert diagnostic ranges from decoded to source coordinates
						for (const diag of diags) {
							if (diag.range) {
								diag.range = convertDecodedToSourceRange(
									mappingCtx,
									diag.range,
								);
							}
				...
				async provideHover(document, position, token) {
					...
					if (mappingCtx) {
						// For JSON: convert source position to decoded position
						const decodedPosition = convertSourceToDecodedPosition(
							mappingCtx,
							position,
						);

						// Get hover at decoded position
						const hover = await mdLs.getHover(document, decodedPosition, token);

						// Convert hover range back to source coordinates
						if (hover?.range) {
							hover.range = convertDecodedToSourceRange(
								mappingCtx,
								hover.range,
							);
						}
			...
			function getMarkdownWorkspace() {
				...
				const workspace: IWorkspace = {
					async getAllMarkdownDocuments() {
						...
						return (
							await Promise.all(
								context.env.workspaceFolders.map(findMarkdownFilesInWorkspace),
							)
						).flat();
					},
					...
					hasMarkdownDocument(resource) {
						...
						return Boolean(
							document && matchDocument(documentSelector, document),
						);
					},
					...
					async stat(resource) {
						...
						return {
							isDirectory: stat?.type === (2 satisfies FileType.Directory),
						};
					},
			...
			async function findMarkdownFilesInWorkspace(folder: URI) {
				...
				await Promise.all(
					files.map(async ([fileName, fileType]) => {
						if (
							fileType === (2 satisfies FileType.Directory) &&
							fileName !== "node_modules"
						) {
							for (const doc of await findMarkdownFilesInWorkspace(
								Utils.joinPath(folder, fileName),
							)) {
								docs.push(doc);
							}
						} else if (
							...
							if (sourceScript?.generated) {
								for (const virtualCode of forEachEmbeddedCode(
									sourceScript.generated.root,
								)) {
									if (matchDocument(documentSelector, virtualCode)) {
										const uri = context.encodeEmbeddedDocumentUri(
											sourceScript.id,
											virtualCode.id,
										);
...
// matchDocument is imported from shared/virtual-code-utils.js
