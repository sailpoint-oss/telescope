...
export interface SchemaResult {
	/** The JSON Schema (TypeBox schemas ARE JSON Schema) */
	jsonSchema: Record<string, unknown>;
	/** Optional TypeBox schema for additional type safety */
 ...
 * - .ts/.js: TypeBox schema exports (loaded via dynamic import)
 *
 * @param filePath - Absolute path to the schema file
 * @param logger - Optional logger for loading operations (defaults to console)
 * @returns Promise resolving to the schema result (JSON schema + optional TypeBox schema)
 */
export async function loadSchema(
	...
	if (extension === ".ts" || extension === ".js" || extension === ".mjs") {
		...
		// Check if it's a default export with a TypeBox schema
		if (
			...
			// TypeBox schemas have certain properties
			if (
				...
				return {
					...
					typeBoxSchema: defaultExport as TSchema,
				};
			}
		}
		...
		logger.warn(
			`Schema from ${filePath} doesn't appear to be a valid JSON Schema or TypeBox schema`,
		);
		return undefined;
	}
