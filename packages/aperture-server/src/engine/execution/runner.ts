 ...
 * @example
 * ```typescript
 * import { runEngine, createRuleContext } from "aperture-server";
 *
 ...
 * @internal
 */
function getLineOffsets(doc: { rawText: string; _lineOffsets?: number[] }): number[] {
	if (!doc._lineOffsets) {
...
function locToRange(
	...
	const start = options?.preferKey && loc.keyStart !== undefined
		? loc.keyStart
		: loc.start;
	const end = options?.preferKey && loc.keyEnd !== undefined
		? loc.keyEnd
		: loc.end;
...
function generateFieldVisitors(
	...
	for (const [visitorName, constraints] of Object.entries(fields)) {
		...
		const validateFields = (ref: { uri: string; pointer: string; node: unknown }) => {
			const node = ref.node as Record<string, unknown> | null | undefined;
			if (!node || typeof node !== "object") return;

			// Check required fields (severity: error)
			if (fieldConstraints.required) {
				for (const [field, message] of Object.entries(fieldConstraints.required)) {
			...
			if (fieldConstraints.suggested) {
				for (const [field, message] of Object.entries(fieldConstraints.suggested)) {
					if (!isFieldPresent(node[field])) {
			...
			if (fieldConstraints.recommended) {
				for (const [field, message] of Object.entries(fieldConstraints.recommended)) {
					if (!isFieldPresent(node[field])) {
...
export function runEngine(
	...
	for (const fileUri of files) {
		...
		const isRootDocument = docAst && typeof docAst === "object" && 
		...
		for (const [pathString, pathItemRefs] of fileIndex.pathsByString.entries()) {
			for (const ref of pathItemRefs) {
				...
				const allPathStrings = fileIndex.pathItemsToPaths.get(key) ?? [pathString];
 ...
 */
function detectOpenAPIVersion(document: { parsed?: unknown }): OpenAPIVersion | "unknown" {
	...
	return {
		...
		report(diag) {
			...
			const codeDescription =
				diag.codeDescription ?? (rule?.meta.url ? { href: rule.meta.url } : undefined);
			diagnostics.push({
				...diag,
				code: compositeCode,
		...
		reportAt(ref, field, opts) {
			...
			let precision: "exact" | "key" | "parent" | "firstChild" | "fallback" = "exact";
			...
			if (!range && doc.ir && fieldPath.length > 1) {
				const lineOffsets = getLineOffsets(doc);
				const escapeSegment = (f: string) => f.replace(/~/g, "~0").replace(/\//g, "~1");
				for (let i = fieldPath.length - 1; i >= 1; i--) {
					...
					const intermediateNode = findNodeByPointer(doc.ir, intermediatePointer);
					if (intermediateNode?.loc) {
						// Found an intermediate parent - highlight its key
						const keyStart = intermediateNode.loc.keyStart ?? intermediateNode.loc.start;
						const keyEnd = intermediateNode.loc.keyEnd ?? intermediateNode.loc.end;
						range = locToRange(
							doc.rawText,
			...
			const codeDescription = rule?.meta.url ? { href: rule.meta.url } : undefined;
			diagnostics.push({
				code: compositeCode,
				source: "telescope",
		...
		locateFirstChild(uri, pointer) {
			...
			const lineOffsets = getLineOffsets(doc);
			return locToRange(doc.rawText, { start: keyStart, end: keyEnd }, lineOffsets);
		},
...
function dispatch(
	...
	switch (kind) {
		...
		case "PathItem":
			...
			break;
		case "Operation":
			enrichedPayload = enrichOperationRef(payload as Parameters<typeof enrichOperationRef>[0]);
			break;
		case "Schema":
			enrichedPayload = enrichSchemaRef(payload as Parameters<typeof enrichSchemaRef>[0]);
			break;
		case "Parameter":
			enrichedPayload = enrichParameterRef(payload as Parameters<typeof enrichParameterRef>[0]);
			break;
		case "Response":
			enrichedPayload = enrichResponseRef(payload as Parameters<typeof enrichResponseRef>[0]);
			break;
		case "RequestBody":
			enrichedPayload = enrichRequestBodyRef(payload as Parameters<typeof enrichRequestBodyRef>[0]);
			break;
		case "Header":
			enrichedPayload = enrichHeaderRef(payload as Parameters<typeof enrichHeaderRef>[0]);
			break;
		case "MediaType":
			enrichedPayload = enrichMediaTypeRef(payload as Parameters<typeof enrichMediaTypeRef>[0]);
			break;
		case "Example":
			enrichedPayload = enrichExampleRef(payload as Parameters<typeof enrichExampleRef>[0]);
			break;
		case "Link":
			enrichedPayload = enrichLinkRef(payload as Parameters<typeof enrichLinkRef>[0]);
			break;
		case "Callback":
			enrichedPayload = enrichCallbackRef(payload as Parameters<typeof enrichCallbackRef>[0]);
			break;
		case "Component":
			enrichedPayload = enrichComponentRef(payload as Parameters<typeof enrichComponentRef>[0]);
			break;
	}
