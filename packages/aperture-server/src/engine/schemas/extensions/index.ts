 ...
 * @example
 * ```typescript
 * import { defineExtension } from "aperture-server";
 *
...
export function compileExtension(meta: ExtensionSchemaMeta): CompiledExtension {
	return {
		...
		url: meta.url,
		compiledSchema: meta.schema(Type),
	};
}
...
export function validateExtensionValue(
	...
	const isValid = Value.Check(extension.compiledSchema, value);
	if (isValid) {
		return { success: true };
	}
	...
	for (const error of Value.Errors(extension.compiledSchema, value)) {
		errors.push({
			path: error.path,
