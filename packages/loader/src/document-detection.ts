...
export function identifyDocumentType(obj: unknown): DocumentType {
	if (!obj || typeof obj !== "object") {
		console.log(
			"[Document Identification] Detected type: unknown (not an object)",
		);
		return "unknown";
	}
	...
	if (typeof data.openapi === "string") {
		console.log(
			`[Document Identification] Detected type: openapi-root (version: ${data.openapi})`,
		);
		return "openapi-root";
	}
	...
	if (rootKey) {
		console.log(
			`[Document Identification] Detected type: openapi-root (root key: ${rootKey})`,
		);
		return "openapi-root";
	}
	...
	if (hasHttpMethod && !data.openapi) {
		console.log("[Document Identification] Detected type: path-item");
		return "path-item";
	}
	...
	if (
		...
		if (data.responses && typeof data.responses === "object") {
			console.log(
				`[Document Identification] Detected type: operation (operationId: ${
	...
	if (data.components) {
		console.log("[Document Identification] Detected type: components");
		return "components";
	}
	...
	if (
		...
		console.log("[Document Identification] Detected type: security-scheme");
	...
	if (
		...
		console.log("[Document Identification] Detected type: schema");
		return "schema";
	}
	...
	if (
		...
		console.log(
			`[Document Identification] Detected type: parameter (name: ${data.name}, in: ${data.in})`,
		);
		return "parameter";
	}
	...
	if (
		...
		console.log("[Document Identification] Detected type: response");
		return "response";
	}
	...
	if (
		...
		console.log("[Document Identification] Detected type: example");
		return "example";
	}

	console.log("[Document Identification] Detected type: unknown");
	return "unknown";
}
...
export function isRootDocument(obj: unknown): boolean {
	if (!obj || typeof obj !== "object") {
		console.log(
			"[Document Identification] isRootDocument: false (not an object)",
		);
		return false;
	}
	const data = obj as Record<string, unknown>;
	...
	if (hasOpenApiVersion) {
		console.log(
			`[Document Identification] isRootDocument: true (version: ${
				data.openapi
			})`,
		);
		return true;
	...
	if (rootKey) {
		console.log(
			`[Document Identification] isRootDocument: true (root key: ${rootKey})`,
		);
		return true;
	}

	console.log("[Document Identification] isRootDocument: false");
	return false;
}
