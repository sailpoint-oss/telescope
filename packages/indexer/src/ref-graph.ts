...
import { getValueAtPointer, isRootDocument, joinPointer } from "loader";
...
export function buildRefGraph(options: BuildRefGraphOptions): RefGraphResult {
		...
		traverse(doc.ast, [], (value, pointer) => {
			...
			if (value && typeof value === "object") {
				...
				if (typeof ref === "string") {
					const target = resolveRef(options.host, uri, ref);
	...
	const resolver = new GraphResolver(options.docs, options.host, nodes);
 ...
 * @param host - The VFS host for resolving relative URIs
		...
		if (value && typeof value === "object") {
			if (Array.isArray(value)) {
				...
				if (typeof obj.$ref === "string") {
					...
					if (refPath?.trim()) {
						// Resolve relative to origin URI
						const resolvedUri = host.resolve(originUri, refPath);
...
function resolveRef(host: VfsHost, fromUri: string, ref: string): GraphNode {
	const [refPath, fragment] = ref.split("#");
	const targetUri = refPath ? host.resolve(fromUri, refPath) : fromUri;
